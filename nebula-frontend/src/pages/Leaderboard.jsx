...
export default function Leaderboard() {
  ...
  useEffect(() => {
    const fetchNodes = async () => {
      ...
      data = data.map(n => ({
        ...n, 
        // Estimate jobs completed based on credits for visual flare, since not tracked directly
        jobsCompleted: Math.floor(n.credits / 15), 
        trend: n.status === 'online' ? 'up' : (n.status === 'failed' ? 'down' : 'stable')
      }));
      data.sort((a, b) => b.trust === a.trust ? b.credits - a.credits : b.trust - a.trust);
      setNodes(data);
    };
    fetchNodes();
