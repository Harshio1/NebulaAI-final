...
export default function Dashboard() {
  const [stats, setStats] = useState({ nodes: 0, cpu: 0, ram: 0, trust: 89 });
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (data.nodes) {
          const totalCpu = data.nodes.reduce((acc, curr) => acc + curr.cpu_cores, 0);
          const totalRam = data.nodes.reduce((acc, curr) => acc + curr.ram_gb, 0);
          const avgTrust = data.nodes.length ? Math.round(data.nodes.reduce((acc, curr) => acc + curr.trust, 0) / data.nodes.length) : 89;
          ...
          const activeJobs = data.recent_jobs.some(j => j.status === 'running' || j.status === 'queued');
          setIsTraining(activeJobs);
        }
      } catch (err) {
  ...
  const handleJoinNetwork = async () => {
    ...
    // Re-fetch immediately after joining
    const data = await api.getClusterStatus();
    if (data.nodes) {
      setStats(prev => ({
        ...prev,
        nodes: data.nodes.length,
        cpu: data.nodes.reduce((acc, curr) => acc + curr.cpu_cores, 0)
      }));
    }
